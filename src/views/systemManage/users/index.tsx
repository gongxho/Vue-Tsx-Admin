      ...
      <div class="layout-container">
        <div class="layout-container-form flex space-between">
          <div class="layout-container-form-handle">
            <el-button type="primary" icon={Plus} onClick={handleAdd}>add</el-button>
            <el-popconfirm
              title="$t('message.common.delTip')"
              onConfirm={() => handleDel(chooseData.value)}
              v-slots={{
                reference: () => (
                  <el-button type="danger" icon={Delete} disabled={chooseData.value.length === 0} >del</el-button>
                )
              }}
            />
          </div>
          <div class="layout-container-form-search">
            ...
            <el-button type="primary" icon={Search} class="search-btn" onClick={() => getTableData(true)} >search</el-button>
          </div>
        </div>
        <div class="layout-container-table">
          ...
          >
            ...
            <el-table-column prop="isAdmin" label="超级管理员" align="center" v-slots={{
              default: (scope: any) => (
                <span class="statusName" style="margin-right:10px">{scope.row.isAdmin === 1 ? "是" : "否"}</span>
              )
            }}>
            </el-table-column>
            <el-table-column prop="status" label="状态" align="center" v-slots={{
              default: (scope: any) => (
                <>
                  <span class="statusName" style="margin-right:10px">{scope.row.status === 1 ? "启用" : "禁用"}</span>
                  <el-switch
